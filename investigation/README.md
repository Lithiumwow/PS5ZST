...
No OSFMount, no `format.com`, no external shell scripts, and no admin rights required.
...
Output: one `.exfat.zst` file + `manifest.txt`

### Option B — Compress an existing image

If you already have a `.exfat` image (e.g. built by `make_image.bat` separately):

```bash
python compress_exfat_pkg.py PPSA12345.exfat out_dir
```

Or build image from folder with a custom command:
```bash
python compress_exfat_pkg.py PPSA12345-app out_dir --build-command "make_image.bat \"{image}\" \"{src}\""
```
